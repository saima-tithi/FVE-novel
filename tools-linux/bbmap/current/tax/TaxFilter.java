...
public class TaxFilter {
	...
	public static TaxFilter makeFilter(String[] args){
		...
		for(int i=0; i<args.length; i++){
			...
			if(a.equals("table") || a.equals("gi")){
				...
				taxLevelE=TaxTree.levelToExtended(TaxTree.parseLevel(b));
		...
		TaxFilter filter=new TaxFilter(tableFile, treeFile, accessionFile, taxLevelE, reqLevels, include, null, regex, contains);
	 ...
	 * @param taxSet_
	 */
	public TaxFilter(TaxTree tree_, int taxLevelE_, int reqLevels_, boolean include_, HashSet<Integer> taxSet_, String regex_, String contains_){
		tree=tree_;
	...
	public TaxFilter(String tableFile, String treeFile, String accessionFile, int taxLevelE_, int reqLevels_, 
			boolean include_, HashSet<Integer> taxSet_, String regex_, String contains_){
		taxLevelE=taxLevelE_;
		reqLevels=reqLevels_;
		include=include_;
		taxSet=(taxSet_==null ? new HashSet<Integer>() : taxSet_);
		...
		while(currentLevelE<TaxTree.LIFE_E){
			...
			for(Integer i : desired){
				if(present.contains(i)){
					taxLevelE=TaxTree.extendedToLevel(currentLevelE);
					if(currentLevelE!=taxLevelE){System.err.println("Widened filter to "+TaxTree.levelToStringExtended(currentLevelE));}
					taxSet=desired;
					return;
				}
			}
