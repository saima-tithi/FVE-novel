			...
			if(ylist!=null){
				for(MicroTile mt : ylist){
					if(mt!=null){
						...
						sb.append(String.format(Locale.ROOT, "%.3f", mt.uniquePercent())).append('\t');
						sb.append(String.format(Locale.ROOT, "%.3f", mt.averageQuality())).append('\t');
						sb.append(String.format(Locale.ROOT, "%.3f", mt.percentErrorFree())).append('\t');
