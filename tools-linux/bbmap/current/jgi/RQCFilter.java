...
//				assert(false) : inPrefix+", "+outPrefix+", "+in1+", "+out1;
				...
				String ref=taxFilter(commonMicrobesRef, "taxa.fa.gz", false, false, true, overwrite, false);
...
//				System.err.println("Filter. step="+step+", in="+in1+", out="+out1+", inPrefix="+inPrefix+", outPrefix="+outPrefix);
				...
				if(chloroMapFlag && taxList!=null){
					...
					TaxTree tree=TaxTree.loadTaxTree(taxTree, System.err, true);
					...
					if(!foundPlant){
						System.err.println("Disabled chloroMapFlag because organism is not a plant.");
						chloroMapFlag=false;
					}
				}
...
//			for(String s : primaryArgList){argList.add(s);}
	 ...
	 */
	private String taxFilter(String in, String out, boolean include, boolean bestEffort, boolean log, boolean ow, boolean app){
		if(taxList==null){
...
//		System.err.println("*Returning "+(fbt.basesOut>0 ? temp : null));
	...
	private String taxFilterList(ArrayList<String> in, String out, boolean include, boolean bestEffort){
		if(in.size()<2){return taxFilter(in.get(0), out, include, bestEffort, true, overwrite, false);}
		
		log("taxFilterList start", true);
		
		for(int i=0; i<in.size(); i++){
			taxFilter(in.get(i), out, include, bestEffort, false, (i==0 && overwrite), i>0);
		}
