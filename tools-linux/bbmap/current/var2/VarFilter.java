...
public class VarFilter {
	...
	public boolean parse(String a, String b, String arg){
		if(a.equals("minreads")){
			...
			maxAvgQuality=Float.parseFloat(b);
			...
			maxAlleleFraction=Float.parseFloat(b);
			...
			maxIdentity=Float.parseFloat(b);
	...
	public void clear(){
		...
		maxAvgQuality=Float.MAX_VALUE;
		maxAvgMapq=Float.MAX_VALUE;
		maxAlleleFraction=Float.MAX_VALUE;
		maxIdentity=Float.MAX_VALUE;
	}
...
//		if(minAvgMapq>0 && v.mapQAvg()<minAvgMapq){return false;}
		...
		if(maxAvgQuality<Float.MAX_VALUE && count*maxAvgQuality<v.baseQSum){return false;}
		if(maxAvgMapq<Float.MAX_VALUE && count*maxAvgMapq<v.mapQSum){return false;}
		if(maxIdentity<Float.MAX_VALUE && count*maxIdentity*1000<v.idSum){return false;}
		...
		if(maxAlleleFraction<Float.MAX_VALUE && v.coverage()>0){
			final float af=v.revisedAlleleFraction==-1 ? v.alleleFraction() : v.revisedAlleleFraction;
			if(af>maxAlleleFraction){return false;}
		}
		
		if(minScore>0 || maxScore<Float.MAX_VALUE){
			float phredScore=v.phredScore(pairingRate, totalQualityAvg, totalMapqAvg, readLengthAvg, rarity, ploidy, map);
	...
	public float maxAvgQuality=Float.MAX_VALUE;
	public float minAvgEdist=10;
	public float minAvgMapq=0;
	public float maxAvgMapq=Float.MAX_VALUE;
	public float minAlleleFraction=0.1f;
	public float maxAlleleFraction=Float.MAX_VALUE;
	public float minIdentity=0f;
	public float maxIdentity=Float.MAX_VALUE;
